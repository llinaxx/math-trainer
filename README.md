# Тренажёр по математике - сложение

## Описание:
Это простое React-приложение, которое помогает практиковать навыки сложения. Пользователь получает случайный пример, вводит ответ и получает обратную связь. Также есть возможность отслеживать успеваемость.

## Основной функционал: 
- Генерация случайных примеров на сложение;
- Проверка ответов;
- Подсчёт правильных и неправильных ответов.

## Скриншоты:
![alt text](main-screen-1.png)
![alt text](correct-answer.png)
![alt text](wrong-answer.png)
![alt text](with-stats.png)
